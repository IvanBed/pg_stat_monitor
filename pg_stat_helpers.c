...
static void 
write_node_name(NodeTag tag, StringInfoData *res_buf)
{
    switch(tag)
    {
...
static void 
write_per_node_instr_info(PlanState const *node, StringInfoData *res_buf)
{
    if (node->instrument)
    { 
...
static void 
dfs_plan_state(PlanState const *node, StringInfoData *res_buf)
{
    if (!node)
        return;
    
    write_node_name(node->type, res_buf);
    write_per_node_instr_info(node, res_buf);
   
    dfs_plan_state(node->lefttree, res_buf);
    dfs_plan_state(node->righttree, res_buf);
}
...
generate_plan_info(QueryDesc const *queryDesc)
{
    ...
    StringInfoData buf;
    initStringInfo(&buf);
    dfs_plan_state(queryDesc->planstate, &buf);
    return buf.data;
}
/*RowLock on orders (transaction 12345) [mode: ExclusiveLock] [wait: 45ms] [relation: orders_pkey]*/
static void 
write_lock_info(LockInstanceData const *instance, LockTagType locktag_type, LOCKMODE mode, TimestampTz end_timestamp, StringInfoData *res_buf)
{
    if (!instance)
    {
        return;
...
write_locks_info(LockData const *locks_data, StringInfoData *buf)
{
    ...
    for (size_t i = 0; i < locks_data->nelements; i++)
    {
        ...
        write_lock_info(instance, (LockTagType) instance->locktag.locktag_type, mode, end_timestamp, buf);        
    }
}
